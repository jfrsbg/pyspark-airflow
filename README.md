# Introdução
Este repositório tem como objetivo resolver um desafio de um processo seletivo para engenharia de dados. No final temos algumas observações, mas boa parte do projeto ja foi disponibilizado previamente (Spark rodando juntamente com airflow no mesmo ambiente em docker).

Conteúdo de minha autoria:
- dags/spark_files/
- dags/pipeline.py
- datalake/staged/
- datalake/refined/
...
## Observações
Em ambiente de produção sabemos que o airflow não pode processar dados, pois ele é apenas um orquestrador, isso nos traria sérios problemas de performance. Precisamos entender que aqui o escopo é uma demo, mas usar colocar o spark com airflow é anti-pattern. Futuramente pretendo evoluir este projeto e colocar um container separado do spark e fazer o airflow apenas dar spark-submit em outro container.


## O que fazer para rodar o projeto
pra rodar o projeto precisa dar permissao total ao diretorio do projeto para que o container do airflow consiga escrever no diretorio nas pastas que serão criadas. Então, basta se posicionar na pasta do projeto e digitar 
```bash
